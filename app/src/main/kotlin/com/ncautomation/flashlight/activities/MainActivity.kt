...
import com.ncautomation.flashlight.BuildConfig
import com.ncautomation.flashlight.R
import com.ncautomation.flashlight.dialogs.SleepTimerCustomAlertDialog
import com.ncautomation.flashlight.extensions.config
import com.ncautomation.flashlight.extensions.startAboutActivity
import com.ncautomation.flashlight.helpers.*
import com.ncautomation.flashlight.screens.*
...
class MainActivity : ComponentActivity() {
    ...
    private fun buildSleepTimerRadioItemsList(
        ...
        sortBy { it.id }
        add(RadioItem(-1, getString(com.ncautomation.commons.R.string.custom)))
    }.toImmutableList()
    ...
    private fun getSleepTimerPermissionDialogState(
        ...
        DialogMember {
            PermissionRequiredAlertDialog(
                alertDialogState = this,
                text = stringResource(id = com.ncautomation.commons.R.string.allow_alarm_sleep_timer),
    ...
    private fun secondsToString(seconds: Int): String {
        ...
        return buildList {
            if (finalHours != 0) {
                add(resources.getQuantityString(com.ncautomation.commons.R.plurals.hours, finalHours, finalHours))
            }
            if (finalMinutes != 0) {
                add(resources.getQuantityString(com.ncautomation.commons.R.plurals.minutes, finalMinutes, finalMinutes))
            }
            if (finalSeconds != 0) {
                add(resources.getQuantityString(com.ncautomation.commons.R.plurals.seconds, finalSeconds, finalSeconds))
            }
